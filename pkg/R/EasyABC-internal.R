...
## function to perform ABC simulations from a non-uniform prior (derived from a
## set of particles)
.ABC_launcher_not_uniformc_cluster_2 <- function(model, prior, param_previous_step, 
    tab_weight, nb_simul, seed_count, inside_prior, n_cluster) {
    tab_simul_summarystat = NULL
    tab_param = NULL
    k_acc = 0
    ...
    npar = floor(nb_simul/(100 * n_cluster))
    n_end = nb_simul - (npar * 100 * n_cluster)
    param_previous_step2 = as.matrix(as.matrix(param_previous_step))
    covwt = 2 * cov.wt(as.matrix(as.matrix(param_previous_step)), as.vector(tab_weight))$cov
    prepare_a_simulation = function() {
        l = dim(param_previous_step)[2]
        counter = 0
        repeat {
          counter = counter + 1
