...
\date{\texttt{EasyABC} version  1.3.99, \Sexpr{Sys.Date()} }
...
\textit{NB:} Note that the fixed variables $J$ and $n\_t$ have been fixed (see section \ref{RC_link}) into the function \texttt{trait\_model} (in versions previous to 1.3, they was included in the prior list using uniform distributions with a trivial ranges)
