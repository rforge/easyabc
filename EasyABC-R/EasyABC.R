...
######################################################################################################################################################
.ABC_Drovandi<-function(model,prior_matrix,nb_simul,summary_stat_target,tolerance_tab=-1,alpha=0.5,c=0.01,first_tolerance_level_auto=TRUE,use_seed=TRUE,seed_count=0,verbose=FALSE){
    ## checking errors in the inputs
    if(!is.vector(tolerance_tab)) stop("'tolerance_tab' has to be a vector.")
    if(tolerance_tab[1]==-1) stop("'tolerance_tab' is missing")
    if (min(tolerance_tab)<=0) stop ("'tolerance values have to be strictly positive.")
...
##############################################################################################################################################
.ABC_Delmoral<-function(model,prior_matrix,nb_simul,summary_stat_target,alpha=0.9,M=1,nb_threshold=floor(nb_simul/2),tolerance_target=-1,use_seed=TRUE,seed_count=0,verbose=FALSE){
...
################################################
.ABC_Lenormand<-function(model,prior_matrix,nb_simul,summary_stat_target,alpha=0.5,p_acc_min=0.05,use_seed=TRUE,seed_count=0,inside_prior=TRUE,verbose=FALSE){
...
#################################################################################################################################################################################################
.ABC_MCMC3<-function(model,prior_matrix,n_obs,n_between_sampling=1,summary_stat_target,n_calibration=10000,tolerance_quantile=0.01,proposal_phi=1,numcomp=0,use_seed=TRUE,seed_count=0,verbose=FALSE){
