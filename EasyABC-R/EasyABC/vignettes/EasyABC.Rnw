...
This last step of simulation rejection can be performed with the \texttt{R} package \texttt{abc}. A worked example demonstrating how the \texttt{EasyABC} and \texttt{abc} functions can be pipelined is provided in section \ref{example}.
...
The original algorithm of Marjoram et al. (2003) is implemented in the method "Marjoram_original" in \texttt{EasyABC}.
...
\begin{itemize}
  ...
  \item  if it is a binary executable file and a single core is used\\
    \texttt{ABC\_rejection(binary\_model("./My\_simulation\_code"), prior\_matrix=priormatrix, nb\_simul=n)}
  \item  if it is a binary executable file and multiple cores are used\\
...
nb_simul=n,summary_stat_target=sum_stat_obs,tolerance_target=tol)
ABC_Delmoral
@
...
Pritchard, J.K., and M.T. Seielstad and A. Perez-Lezaun and M.W. Feldman (1999) Population growth of human Y chromosomes: a study of Y chromosome microsatellites. \emph{Molecular Biology and Evolution}, \bold{16}, 1791--1798.

Sisson, S.A., Fan, Y., and Tanaka, M.M. (2007) Sequential Monte Carlo without likelihoods. \emph{PNAS}, \bold{104}, 1760--1765.

Venables, W.N., and Ripley, B.D. (2002) Modern Applied Statistics with S. Fourth Edition. Springer, New York.

Wegmann, D., Leuenberger, C. and Excoffier, L. (2009a) Efficient approximate Bayesian computation coupled with Markov chain Monte Carlo without likelihood. \emph{Genetics}, \bold{182}, 1207-1218.
