.ABC_Delmoral <-
function(model,prior_matrix,nb_simul,alpha,M,nb_threshold,tolerance_target,summary_stat_target,use_seed=TRUE,seed_count=0){
...
.ABC_Drovandi <-
function(model,prior_matrix,nb_simul,tolerance_tab,summary_stat_target,alpha=0.5,c=0.01,first_tolerance_level_auto=TRUE,use_seed=TRUE,seed_count=0){
...
.ABC_launcher_not_uniform <-
function(model,prior_matrix,param_previous_step,tab_unfixed_param,tab_weight,nb_simul,use_seed,seed_count,inside_prior){
...
.ABC_Lenormand <-
function(model,prior_matrix,nb_simul,summary_stat_target,alpha=0.5,p_acc_min=0.05,use_seed=TRUE,seed_count=0,inside_prior=TRUE){
	nparam=dim(prior_matrix)[1]
	nstat=length(summary_stat_target)
	tab_unfixed_param=array(TRUE,nparam)
	for (i in 1:nparam){
...
.ABC_MCMC <-
function(model,prior_matrix,n_obs,n_between_sampling,summary_stat_target,dist_max,tab_normalization,proposal_range,use_seed=TRUE,seed_count=0){
	nparam=dim(prior_matrix)[1]
	nstat=length(summary_stat_target)
...
.ABC_MCMC2 <-
function(model,prior_matrix,n_obs,n_between_sampling,summary_stat_target,n_calibration=10000,tolerance_quantile=0.01,proposal_phi=1,use_seed=TRUE,seed_count=0){
	nparam=dim(prior_matrix)[1]
	nstat=length(summary_stat_target)
	tab_simul_summary_stat=NULL
	tab_param=NULL
...
.ABC_MCMC3 <-
function(model,prior_matrix,n_obs,summary_stat_targ,n_calibration=10000,tolerance_quantile=0.01,proposal_phi=1,n_between_sampling=1,numcomp=0,use_seed=TRUE,seed_count=0){
##AM1
	nparam=dim(prior_matrix)[1]
...
.ABC_PMC <-
function(model,prior_matrix,nb_simul,tolerance_tab,summary_stat_target,use_seed=TRUE,seed_count=0,inside_prior=TRUE){
...
.ABC_rejection_lhs <-
function(model,prior_matrix,nb_simul,tab_unfixed_param,use_seed=TRUE,seed_count=0){
	tab_simul_summarystat=NULL
	tab_param=NULL
...
.ABC_rejection_M <-
function(model,prior_matrix,nb_simul,M,use_seed,seed_count){
	tab_simul_summarystat=NULL
	tab_param=NULL
...
.compute_below <-
function(particle_dist_mat,tolerance){
...
.compute_dist <-
function(summary_stat_target,simul,sd_simul){
...
.compute_dist_M <-
function(M,summary_stat_target,simul,sd_simul){
	l=length(summary_stat_target)
	nsimul=dim(simul)[1]
	vartab=array(1,l)
	dist=array(0,nsimul)
...
.compute_weight <-
function(param_simulated,param_previous_step,tab_weight){
...
.compute_weightb <-
function(param_simulated,param_previous_step,tab_weight,prior_density){
	vmat=2*var(param_previous_step)
	n_particle=dim(param_previous_step)[1]
	n_new_particle=dim(param_simulated)[1]
	tab_weight_new=array(0,n_new_particle)
...
.move_particle <-
function(param_picked,varcov_matrix,prior_matrix){
...
.move_particleb_uni <-
function(param_picked,sd_array,prior_matrix){
	test=FALSE
	res=param_picked
	while (!test){
...
.selec_simul <-
function(summary_stat_target,param,simul,sd_simul,tol){
...
.selec_simul_alpha <-
function(summary_stat_target,param,simul,sd_simul,alpha){
	dist=.compute_dist(summary_stat_target,simul,sd_simul)
